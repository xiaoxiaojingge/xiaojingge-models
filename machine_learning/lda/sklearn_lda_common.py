...
@Author  : lijing
@File    : sklearn_lda_no_view.py
@Description: 通用的 LDA 主题提取和可视化完整代码
---------------------------------------
"""
...
df = (
    pd.read_csv(source_csv_path, encoding="utf-8-sig")
    .drop_duplicates()
...
df["cut"] = (
    ...
    .apply(
        lambda x: " ".join(
            [word for word in jieba.lcut(x) if word not in stop_words_set]
        )
    )
)
...
print("本次生成了文件：", top_words_csv_path, predict_topic_csv_path, html_path)
