...
def train():
    ...
    print("###加载训练集ing...###")
    train_loader = DataLoader(TextDataset('./dataset/train.csv', tokenizer), batch_size=config.batch_size, shuffle=True, num_workers=config.num_workers)
    print("###加载验证集ing...###")
    val_loader = DataLoader(TextDataset('./dataset/valid.csv', tokenizer), batch_size=config.batch_size, shuffle=True, num_workers=config.num_workers)
    for epoch in range(config.num_epochs):
        ...
        with open('./products/epoch_loss.txt', 'a') as f:
            f.write(f"Epoch {epoch + 1}, Start Time: {epoch_start_time}, Train Loss: {train_losses[-1]:.4f}, Val Loss: {val_losses[-1]:.4f}\n")
