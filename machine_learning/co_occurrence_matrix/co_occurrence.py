...
"""
---------------------------------------
@Time    : 2024-08-10 13:52
@Author  : lijing
@File    : co_occurrence.py
@Description: 
---------------------------------------
"""
...
def generate_cooccurrence_matrix(cooccurrence_matrix, top_words):
    ...
    sns.heatmap(
        ...
        yticklabels=[word for word, _ in top_words],
    )
    plt.title("共现矩阵热力图")
    plt.xlabel(" ")
    plt.ylabel(" ")
...
def generate_cooccurrence_network(cooccurrence_matrix, min_threshold, word_to_index):
    ...
    G.remove_edges_from(
        [(u, v) for u, v, d in G.edges(data=True) if d["weight"] < min_threshold]
    )

    # 定义函数来获取节点的标签（词汇）
    def get_node_label(node_id):
        return word_to_index.get(
            node_id, str(node_id)
        )  # 如果找不到对应的词汇，返回节点ID作为备选
    ...
    plt.title("共现矩阵关系图")
    plt.rcParams["font.sans-serif"] = ["SimHei"]  # 使用黑体作为字体
    plt.rcParams["axes.unicode_minus"] = False  # 正常显示负号
