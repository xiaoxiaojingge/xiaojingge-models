...
# 必要参数定义
# 训练设备,如果NVIDIA GPU已配置，会自动使用GPU训练
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
train_ratio = 0.8  # 训练集比例
val_ratio = 0.1  # 验证集比例
test_ratio = 0.1  # 测试集比例
batch_size = 64  # 批大小，若用CPU，建议为1
input_length = 35  # 每个batch的输入数据长度，多步预测建议长，单步预测建议短
...
dim = 128  # 隐层维度
interval_length = 7000  # 预测数据长度，最长不可以超过总数据条数
scalar = True  # 是否使用归一化
scalar_contain_labels = True  # 归一化过程是否包含目标值的历史数据
target_value = "value"  # 需要预测的列名，可以在excel中查看
# 多步，单步标签
if output_length > 1:
...
# 初步划分训练集、验证集、测试集
split_train_val, split_val_test = int(len(features_) * train_ratio), int(
    len(features_) * train_ratio
) + int(len(features_) * val_ratio)
...
split_train_val, split_val_test = int(len(features) * train_ratio), int(
    len(features) * train_ratio
) + int(len(features) * val_ratio)
train_features, train_labels = features[:split_train_val], labels[:split_train_val]
val_features, val_labels = (
    features[split_train_val:split_val_test],
...
optimizer = torch.optim.AdamW(
    LSTMMain_model.parameters(), lr=learning_rate, weight_decay=weight_decay
)
...
for epoch in range(epochs):
    ...
    if epoch == 0:
        ...
        if val_best > val_loss_sum:
            val_best = val_loss_sum
            torch.save(
                LSTMMain_model.state_dict(), "./products/model_lstm.weights"
            )  # 保存最好权重
...
#  测试集预测
LSTMMain_model.load_state_dict(torch.load("./products/model_lstm.weights"))  # 调用权重
test_loss_sum = 0
#  测试集inference
...
if scalar_contain_labels and scalar:
    ...
    if features_num == 1 and output_length == 1:
        for pre_slice in range(pre_array.shape[0]):
            pre_inverse_slice = scalar.inverse_transform(
                np.expand_dims(pre_array[pre_slice, :], axis=1)
            )
            test_inverse_slice = scalar.inverse_transform(
                np.expand_dims(label_array[pre_slice, :], axis=1)
            )
        ...
        for pre_slice in range(pre_array.shape[0]):
            pre_inverse_slice = scalar.inverse_transform(
                torch.cat(
                    (
                        torch.zeros(pre_array[0].shape[0], features_num - 1),
            ...
            test_inverse_slice = scalar.inverse_transform(
                torch.cat(
                    (
                        torch.zeros(test_labels[0].shape[0], features_num - 1),
