...
#  模型定义
LSTMMain_model = LSTMMain(input_size=features_num, output_len=output_length, lstm_hidden=dim, lstm_layers=num_blocks, batch_size=batch_size, device=device)
...
for epoch in range(epochs):
    ...
    if epoch == 0:
        ...
        if val_best > val_loss_sum:
            val_best = val_loss_sum
            torch.save(LSTMMain_model.state_dict(), './products/model_lstm.weights')  # 保存最好权重
