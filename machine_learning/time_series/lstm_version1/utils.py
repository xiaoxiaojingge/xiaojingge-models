...
def get_rolling_window_multistep(
    forecasting_length, interval_length, window_length, features, labels
):
    ...
    if features.shape[1] != labels.shape[1]:
        ...
        for index in tqdm.tqdm(
            ...
            output_features = np.concatenate(
                (
                    output_features,
                    np.expand_dims(features[:, index : index + window_length], axis=0),
                )
            )
