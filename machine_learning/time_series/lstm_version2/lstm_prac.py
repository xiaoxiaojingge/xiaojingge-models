...
dis_std = np.std(test_df["discharge"].values)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if train_mean_loss < 0.2 and test_mean_loss < min_test_loss:
        ...
        torch.save(model, "./products/lstm_model.pth")
...
model = torch.load("./products/lstm_model.pth", map_location=torch.device("cuda"))
model.eval()
