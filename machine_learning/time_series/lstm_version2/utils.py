...
class Cutter(Dataset):
    ...
    def fit(self, df):
        ...
        for col in self.cols:
            ...
            qcut_ret = pd.qcut(
                cur_series, self.bins, labels=False, retbins=True, duplicates="drop"
            )
    ...
    def transform(self, df):
        ...
        for col in self.cols:
            ...
            q_res = pd.cut(
                df[col],
                save_num_bins,
                labels=range(0, len(save_num_bins) - 1),
                retbins=True,
                include_lowest=True,
            )
            df[col] = (
                q_res[0]
                .cat.add_categories(len(save_num_bins) - 1)
                .fillna(len(save_num_bins) - 1)
                .astype("int")
            )
...
class MyLSTM_AM(nn.Module):
    def __init__(
        ...
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True)
        self.lstmcell = nn.LSTMCell(input_size=hidden_dim, hidden_size=hidden_dim)
    ...
    def Atten(self, H):
        ...
        atten = self.mysoftmax(torch.matmul(h, H.transpose(1, 2)).transpose(1, 2))
    ...
    def forward(self, x):
        ...
        h = h.squeeze(0)
        c = c.squeeze(0)
        H_pre = torch.empty((h.shape[0], self.predict_len, self.hidden_dim * 2)).cuda()
