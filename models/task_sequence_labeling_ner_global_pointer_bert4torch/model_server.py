...
class Model(BaseModel):
    ...
    def __init__(self, params: dict):
        super().__init__()
        self.bert = build_transformer_model(config_path=params['bert_config_path'],
                                            checkpoint_path=params['bert_checkpoint_path'],
                                            segment_vocab_size=0)
        self.global_pointer = GlobalPointer(hidden_size=768, heads=params['ner_vocab_size'],
...
class Evaluator(Callback):
    ...
    def on_epoch_end(self, steps, epoch, logs=None):
        ...
        f1, precision, recall = self.model_server.evaluate(self.model_server.valid_dataloader)
        if f1 > self.best_val_f1:
            self.best_val_f1 = f1
            # 保存更优f1的模型
            self.model_server.model.save_weights(self.model_server.model_dir + '/models/best_model.weights')
            final_f1 = round(f1, 4)
...
class ModelServer:
    ...
    def __init__(self, model_dir: str, sign: str):
        ...
        # 训练集
        if sign == 'train':
            self.train_dataloader = DataLoader(MyDataset(model_dir + '/train.bio'),
                                               batch_size=self.batch_size, shuffle=True, collate_fn=self.collate_fn)
            # 验证集
            self.valid_dataloader = DataLoader(MyDataset(model_dir + '/valid.bio'),
                                               batch_size=self.batch_size, collate_fn=self.collate_fn)

        self.model.compile(loss=MyLoss(), optimizer=optim.Adam(self.model.parameters(), lr=self.learning_rate))
    ...
    def collate_fn(self, batch):
        ...
        for i, (text, text_labels) in enumerate(batch):
            ...
            labels = np.zeros((len(self.categories_label2id), self.max_len, self.max_len))
        ...
        batch_labels = torch.tensor(sequence_padding(batch_labels, seq_dims=3), dtype=torch.long, device=self.device)
    ...
    def inference(self, text: str, threshold=0):
        ...
        for l, start, end in zip(*np.where(scores[0].cpu() > threshold)):
            res[self.tokenizer.decode(tokens[start:end + 1])] = self.categories_id2label[l]
        return res
    ...
    def start_train(self, result_queue: Queue):
        ...
        try:
            ...
            logger.info(f'命名实体识别模型开始训练，模型训练以及语料存储路径为：{self.model_dir}')
            self.model.fit(self.train_dataloader, epochs=self.epochs, steps_per_epoch=None, callbacks=[evaluator])
            ...
            self.email_server.send_email_2_admin('命名实体识别模型训练过程中出现异常',
                                                 '【task_sequence_labeling_ner_global_pointer_bert4torch】训练过程中出现异常，停止训练！ -> {}'.format(
                                                     e))
            logger.error('命名实体识别模型训练过程中出现异常，停止训练！ -> {}'.format(e))
            # # 指定错误标志
            result = {
                "sign": 'train',
                "code": 500,
                "message": '命名实体识别模型训练过程中出现异常，停止训练！ -> {}'.format(e),
                "data": {}
            }
            result_queue.put(result)
        finally:
    ...
    def start_predict(self, predict_contents, result_queue: Queue):
        ...
        try:
            ...
            self.email_server.send_email_2_admin('命名实体识别模型预测过程中出现异常',
